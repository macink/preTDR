...
void preTDR_incoherent_production(TString coherentFile, TString incoherentFile)
{
    TFile* coherent_phi_file = TFile::Open(coherentFile, "READ");
    TFile* incoherent_phi_file = TFile::Open(incoherentFile, "READ");

	TH1D* h_t_MC = (TH1D*) coherent_phi_file->Get("h_t_MC");
	TH1D* h_t_REC_wRES_cut_pi12 = (TH1D*) coherent_phi_file->Get("h_t_REC_wRES_cut_pi12");
    TH1D* h_phi_sartre_events = (TH1D*) coherent_phi_file->Get("h_Nevents");

    TH1D* h_incoherent_phi_noVetoes_proj = (TH1D*) incoherent_phi_file->Get("h_t_REC_wRES_cut_pi12");
    TH1D* h_incoherent_events = (TH1D*) incoherent_phi_file->Get("h_Nevents");
    ...
    c14_213->Print("./plot_t_incoherent_noVetoes_preTDR.pdf");
}
